...
class PrettyDioLogger extends Interceptor {
  ...
  String _indent([int tabCount = 1]) => tabStep * tabCount;
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        ...
        if (compact && _canFlattenList(value)) {
          logPrint(type, '║${_indent(tabs0)} $key: ${value.toString()}');
        } else {
          logPrint(type, '║${_indent(tabs0)} $key: [');
          _printList(type, value, tabs: tabs0);
          logPrint(type, '║${_indent(tabs0)} ]${isLast ? "" : ','}');
        }
      } else {
        ...
        if (msg.length + indent.length > linWidth) {
          ...
          logPrint(type, '║${_indent(tabs0)} $key: $msg${!isLast ? ',' : ""}');
        }
      }
    });
  ...
  void _printList(_DebugTypes type, List list, {int tabs = 1}) {
